    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="buttonAdd" class="btn btn-primary" onclick="submit4GBBU()">
                    提交更改
                </button>
            </div>
        </div><!-- /.modal-content -->
...
<button type="button" onclick="openAdd4GRRU()" class="btn btn-info glyphicon glyphicon-plus">新增</button>
<button type="button" onclick="del4GRRU()" class="btn btn-danger glyphicon glyphicon-minus">删除</button>
...
<script type="text/javascript">
    ...
    function initRRU(){
        $('#4GRRUTable').bootstrapTable('destroy');
        $("#4GRRUTable").bootstrapTable({
            ...
            columns:[
                ...
                {field:'sign',title:'操作' ,class:'table-width',width:'10%',formatter:function(value,row,index){
                        return  ' <a href="javascript:editRRu('+row.id+',\'' + row.dxCode + '\',\'' + row.rruCode + '\',\'' + row.rruName + '\',\'' + row.netCareId + '\',\'' + row.netCareName + '\');">修改</a>  ';
                    }}
            ]
        })
    }
    function openAdd4GRRU(){
        $("#id").val("");
        $("#dxCode").val("");
        $("#rruCode").val("");
        $("#rruName").val("");
    ...
    //批量删除
    function del4GRRU(){
        var arr = $('#4GRRUTable').bootstrapTable('getSelections');
        if (arr.length <= 0) {
            bootbox.alert({
                size: "small",
                title: "提示",
    ...
    function EXPContract(){
        ...
        bootbox.confirm({
            ...
            callback: function(result){
                ...
                location.href="/poi/createExcel?ids="+ids+"&&type=6"
            }
        })
    }
    //打开导入弹框
